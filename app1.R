...
#this version use files for both,but file result is different
...
compute_and_plot_z <- function(data, mode, mouse_id, date_range, retain, circadian, flag, exp_select, selected_mode) {
  ...
  mouse_data <- mouse_data %>%
    left_join(mean_sd_data, by = character()) %>%
    mutate(across(all_of(selected_vars),
                  ~ (. - get(paste0(cur_column(), "_mean"))) / get(paste0(cur_column(), "_sd")),
                  .names = "z_{col}"))
  ...
  z_summary_vars <- grep(paste0("_", mode, "$"), names(combined_data_z), value = TRUE)
  z_score_long_combined <- combined_data_z %>%
    select(Mouse, all_of(z_summary_vars)) %>%
    tidyr::pivot_longer(-Mouse, names_to = "variable", values_to = "z_score")
  
  # Trim variable names
  z_score_long_combined <- z_score_long_combined %>%
    mutate(variable = gsub(paste0("_z_", mode), "", variable))
...
server <- function(input, output, session) {
  ...
  #rename to min so the display can still be easier (this version apply new file for display)
  columns_to_modify <- c("Regular_Water_Beambreak", "Sucrose_Water_Beambreak", "Regular_Food_Beambreak", 
                         "Fatty_Food_Beambreak", "Regular_Water_Dispense", "Sucrose_Water_Dispense", 
                         "Regular_Food_Dispense", "Fatty_Food_Dispense","Wheel_Beambreak_CM")
  
  analog <- reactive({
    exp <- exp_reactive()
    if (exp == "Exp6") {
      data<-read.csv("Exp6_analog_MINUTES_ALL_July24_2024.csv")
      data$Mouse <- format_mouse_numbers(data$Mouse)
      colnames(data)[colnames(data) %in% columns_to_modify] <- paste0(colnames(data)[colnames(data) %in% columns_to_modify], "_min")
      data %>%
        mutate(hour = format(as.POSIXct(time), "%Y-%m-%d %H:00:00")) %>%
        group_by(hour) %>%
        mutate(across(ends_with("_min"), sum, .names = "{str_remove(.col, '_min')}")) %>%
        ungroup()
    } else if (exp == "Exp5") {
