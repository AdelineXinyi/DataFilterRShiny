...
create_multi_mouse_plot <- function(unfiltered_data, data, mode, var_stem, plot_type,
  ...
  plots <- lapply(seq_along(mouse_ids), function(i) {
    ...
    plot <- NULL
    if (mode == "z") {
      plot <- compute_and_plot_z(unfiltered_data, z_mode_select, mouse_id, date_range, retain, circadian, flag, exp_select, selected_mode)
    } else {
      mouse_data <- data %>% filter(Mouse == mouse_id)
      if (plot_type == "arrow") {
        plot <- create_arrow_plot(mouse_data, var_stem, date_range, circadian, flag, exp_select, selected_mode)
      } else {
        plot <- create_bar_plot(mouse_data, var_stem, date_range, circadian, flag, exp_select, selected_mode)
      }
    }
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      actionButton("downloadD", "Download Data"),
...
server <- function(input, output, session) {
  ...
  format_mouse_numbers <- function(numbers) {
    formatted_numbers <- ifelse(numbers < 10,
                                sprintf("BB%02d", numbers),
                                sprintf("BB%d", numbers))
  ...
  am <- reactive({
    exp <- exp_reactive()
    if (exp == "Exp6") {
      data<-read.csv("Exp6_analog_MINUTES_ALL_July24_2024.csv")
    } else if (exp == "Exp5") {
      data<-read.csv("Exp5_analog_MINUTES_ALL_July24_2024.csv")
    }
  })
  dm <- reactive({
    exp <- exp_reactive()
    if (exp == "Exp6") {
      data<-read.csv("Exp6_digital_MINUTES_ALL_July24_2024.csv")
    } else if (exp == "Exp5") {
      data<-read.csv("Exp5_digital_MINUTES_ALL_July24_2024.csv")
    }
  })
  ...
  mina <- reactive({
    ...
    ana_data$day <- as.numeric(gsub("D", "", as.character(ana_data$day)))
    digi_data$day <- as.numeric(gsub("D", "", as.character(digi_data$day)))
    ana_data$Mouse <- format_mouse_numbers(ana_data$Mouse)
  ...
  mind<-reactive({
    ...
    ana_data$day <- as.numeric(gsub("D", "", as.character(ana_data$day)))
    digi_data$day <- as.numeric(gsub("D", "", as.character(digi_data$day)))
    digi_data$Mouse <- format_mouse_numbers(digi_data$Mouse)
  ...
  d_data <- reactive({
    ...
    suffix <- switch(input$z_mode_select,
                     "sum" = "_sum$",
                     "median" = "_median$",
                     "mean" = "_mean$")
    ...
    z_score_long_combined <- z_score_long_combined %>%
      mutate(variable = gsub("z_mean_|_sum$|_median$|_mean$", "", variable))
    z_score_long_combined %>%
      filter(Mouse %in% input$mouse_id)
  })
  
  #original master after mouse,day
  #bar plot filter?
  hr_data <- reactive({
    ana_data <- ana()
    digi_data <- digi()
  ...
  #add column for circadian/flag or make it dataset name
  day_data <- reactive({
    date_range<-selected_date_range()
    mouse_data <- data()%>%
      group_by(Mouse) %>%
  ...
  ddata <- reactive({
    print(paste("Selected display type:", input$display)) 
  ...
  observeEvent(input$downloadD, {
    showModal(modalDialog(
      title = "Select Data to Download",
      pickerInput(
        inputId = "display", 
        label = "Select Data Type", 
        choices = c("Display"="d","Min Data" = "min", "Hour Data" = "hr", 
                    "Day Data" = "day"), 
        selected = "d",
        multiple = FALSE
      ),
  ...
  #############################download####################################
  #change the name by passing circadian and flag info
  output$downloadData <- downloadHandler(
    filename = function() {
      if (input$display != "min") {
        paste(input$exp_select, "-", input$circadian, "-", input$flag, ".csv", sep = "")
      } else {
        paste(input$exp_select, "-", input$circadian, "-", input$flag, ".zip", sep = "")
      }
    },
    content = function(file) {
      if (input$display != "min") {
        ...
        ana_file <- "ana_data.csv"
        digi_file <- "digi_data.csv"
        
        ana_file_path <- file.path(temp_dir, ana_file)
        digi_file_path <- file.path(temp_dir, digi_file)
        
        # Write the CSV files
        write.csv(min_a(), ana_file_path, row.names = FALSE)
        write.csv(min_d(), digi_file_path, row.names = FALSE)
        
        old_wd <- setwd(temp_dir)
        on.exit(setwd(old_wd))  # Ensure we return to the original directory
        
        # Create a zip file containing both CSV files
        zip::zip(file, files = c(ana_file, digi_file))
      }
    }
  )
